...
namespace ProgressionExpanded
{
    ...
    internal sealed class DestituteStartBehavior : CampaignBehaviorBase
    {
        ...
        /// <summary>Whether the kit has already been paid for the first body searched.</summary>
        private bool _kitGiven;

        /// <summary>
        /// Swaps a searched body's gear for the knife and the boots, once per campaign.
        /// </summary>
        /// <remarks>
        /// Not tied to the opening window: the mission that pays this way happens well after the
        /// tutorial has finished, so anything that stopped at the tutorial would never fire.
        ///
        /// Takes back only what the search itself added, by comparing the packs against a count
        /// taken a moment before. Sweeping them wholesale would be fine during the opening and
        /// theft afterwards, when the packs hold things the player went and bought.
        /// </remarks>
        internal void TakeTheBodysGear(Dictionary<string, int> before)
        {
            ...
            Log.Write("Burlap sack: took " + string.Join(", ", taken.ToArray())
                      + $" and left {knife?.StringId} (tier {knife?.Tier}), {shoes?.StringId} (tier {shoes?.Tier})");
        }
    ...
    /// <summary>
    /// Replaces what a searched body gives up with a knife and a pair of boots.
    /// </summary>
    /// <remarks>
    /// This is the reward that was arriving as a full set: the stealth mission against the bandits
    /// lets the player search the bodies, and each search drops that bandit's gear into the packs.
    /// It is not a quest reward in any obvious sense, which is why it took so long to find - the
    /// tutorial's own ending hands over two grain and nothing else.
    ///
    /// The search itself is left alone and its takings are swapped afterwards, so the prompt, the
    /// animation and the game's own bookkeeping all still happen. The kit is given once, on the
    /// first body: searching four of them should not produce four daggers.
    /// </remarks>
    [HarmonyPatch]
    internal static class SearchBodyLootPatch
    {
        private const string Handler = "SandBox.Missions.MissionLogics.SearchBodyMissionHandler";

        private static bool Prepare() => Target() != null;

        private static MethodBase? Target() =>
            AccessTools.Method(AccessTools.TypeByName(Handler), "AddItemsToPlayer");

        private static MethodBase TargetMethod() => Target()!;

        /// <summary>What the packs held a moment before the body was searched.</summary>
        private static readonly Dictionary<string, int> Before = new Dictionary<string, int>();

        [HarmonyPrefix]
        private static void Count()
        {
            Guard.Touch("SearchBody");
            ...
            {
                Guard.Report("SearchBody.Count", exception);
            }
        }
        ...
        private static void Swap()
        {
            try
            {
                DestituteStartBehavior.Current?.TakeTheBodysGear(Before);
            }
            catch (Exception exception)
            {
                Guard.Report("SearchBody", exception);
            }
        }
    }
}
